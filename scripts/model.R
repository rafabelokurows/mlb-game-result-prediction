df = read.csv("20230516games.csv")
predictors = c("teams.away.leagueRecord.pct","teams.home.leagueRecord.pct",
               "wins_last5",
               ...
               "home_pitcher_mean_ip",
               "away_pitcher_mean_ip",
               "away_pitcher_winpct",
               "home_pitcher_winpct",
               "home_dif_win_pct",
               "away_dif_win_pct", "last30_away_pitcher_winpct",
...
sample <- sample.split(df$officialDate, SplitRatio = 0.85)
...
automl_model <- h2o.automl(
  ...
  #stopping_metric = "AUC", # metric to use for early stopping
  sort_metric = "mean_per_class_error", # metric to sort models by in leaderboard
  nfolds = 5, # number of cross-validation folds
  include_algos = c("GBM", "DRF", "XGBoost", "DeepLearning","GLM"), # list of algorithms to include
